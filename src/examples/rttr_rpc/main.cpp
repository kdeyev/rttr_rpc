...
int main(int argc, char** argv) {
    ...
    m = jsonrpc::parser::parse_json(
        nlohmann::json::parse(R"({"jsonrpc": "2.0", "method": "test.func2", "params": [{"x":42,"y":41}, {"x":41,"y":42}], "id": 2})"));
    response = repo.process_message(m);
    std::cout << response->to_json().dump(4) << std::endl;

    m = jsonrpc::parser::parse_json(
        nlohmann::json::parse(R"({"jsonrpc": "2.0", "method": "test.func", "params": [{"x":42,"y":41}, {"x":41,"y":42}], "id": 2})"));
    response = repo.process_message(m);
    std::cout << response->to_json().dump(4) << std::endl;
